...
<link href="requestlist/css/bootstrap.min.css" rel="stylesheet" media="screen">
      ...
      <form action="index.php?m=form&a=ncap&pid=<?=$_GET['pid']?>" method="post" enctype="multipart/form-data" id="form1" name="form1">
        <table width="100%" border="0">
		<? 
			$qcekreqaprvlx = mysql_query("SELECT ticketapprovalstatus_id,ticketapprovalstatus_id2,ticketapprovalstatus_id3 
			FROM ITH_TICKET_HEADER WHERE ticket_id = '$_GET[pid]'");	
			$rcekreqaprvlx = mysql_fetch_object($qcekreqaprvlx);
			/*if(($rcekreqaprvlx->ticketapprovalstatus_id=='2')||($rcekreqaprvlx->ticketapprovalstatus_id2=='2')||
			($rcekreqaprvlx->ticketapprovalstatus_id3=='2')){	*/
		?>
		<tr>
		 <td valign="top">Request Approved :  <span class="note" style="color:red;">*</span></td>
            <td valign="top">
		<? 
			#if($usrlevel == '1001'){		
				if($ticket_id!='') {
					$qreqaprvlx = mysql_query("SELECT ticketapprovalstatus_id FROM ITH_TICKET_HEADER WHERE ticket_id = '$_GET[pid]'");	
					$rreqaprvlx = mysql_fetch_object($qreqaprvlx);	
		?>			<?if(($rreqaprvlx->ticketapprovalstatus_id=='0')){?>
								<input type="radio" name="ticketapprovalstatus_id"  value="<?='2'?>">&nbsp;<?='Has not responded'?>&nbsp;
								<input type="radio" name="ticketapprovalstatus_id" value="<?='0'?>" checked>&nbsp;<?='Un-Approval'?>&nbsp;
								<input type="radio" name="ticketapprovalstatus_id" value="<?='1'?>">&nbsp;<?='Approval'?>&nbsp;
					<?}elseif(($rreqaprvlx->ticketapprovalstatus_id=='1')){?>
								<input type="radio" name="ticketapprovalstatus_id"  value="<?='2'?>">&nbsp;<?='Has not responded'?>&nbsp;
								<input type="radio" name="ticketapprovalstatus_id" value="<?='0'?>">&nbsp;<?='Un-Approval'?>&nbsp;
								<input type="radio" name="ticketapprovalstatus_id" value="<?='1'?>" checked>&nbsp;<?='Approval'?>&nbsp;
					<?}elseif(($rreqaprvlx->ticketapprovalstatus_id=='2')||($rreqaprvlx->ticketapprovalstatus_id=='3')){?>
								<input type="radio" name="ticketapprovalstatus_id"  value="<?='2'?>" checked>&nbsp;<?='Has not responded'?>&nbsp;
								<input type="radio" name="ticketapprovalstatus_id" value="<?='0'?>">&nbsp;<?='Un-Approval'?>&nbsp;
								<input type="radio" name="ticketapprovalstatus_id" value="<?='1'?>">&nbsp;<?='Approval'?>&nbsp;
					<?}else{?>
								<? 
									$qto = mysql_query("SELECT ticketstatusapproval_id,ticketstatusapproval_name FROM ITH_TICKETSTATUSAPRVL
									WHERE ticketstatusapproval_id NOT IN('3') ORDER BY ticketstatusapproval_id ASC");
									while($rto = mysql_fetch_object($qto)) {
								?>
								 <input type="radio" name="ticketapprovalstatus_id" class="required" value="<?=$rto->ticketstatusapproval_id?>">&nbsp;<?=$rto->ticketstatusapproval_name?>&nbsp;
		...
		<? #} ?>
          ...
          <tr>
			...
			<?
			if($ticketdetail_id!='') {
					$qreqnoteaprvlx = mysql_query("SELECT ticketnote_picto1 FROM ITH_LOGREPORT WHERE ticket_id = '$_GET[pid]' AND ticketapproval_bynik1 = '$usrnik' ");	
			...
			?>
				<?if(($rreqnoteaprvlx->ticketnote_picto1!='')){?>
				
					<td valign="top"><textarea name="ticketnote_picto1" id="ticketnote_picto1" rows="15" value="<?=$rreqnoteaprvlx->ticketnote_picto2;?>" class="input"><?=$rreqnoteaprvlx->ticketnote_picto2;?></textarea></td>
				<?}else{?>
					<td valign="top"><textarea name="ticketnote_picto1" id="ticketnote_picto1" rows="15" class="input"></textarea></td>
				<? } ?>	
			<? } ?>
          </tr>	
