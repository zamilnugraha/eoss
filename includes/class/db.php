<?php
	class mysqldb {
		...
		function connect($host,$user,$pass,$dbase) {
			...
			$sqlconnect = mysql_connect($host,$user,$pass) or die($msgSqlConnect);
			if ($sqlconnect) {
				 $sqldb = mysql_select_db($dbase,$sqlconnect) or die($msgSelectDB);
		...
		function runQuery($str) {
			...
			$result = mysql_query($str) or die($msgResult);
			return $result;
		}
		
		function getDataRow($objectResult) {
			$row = mysql_fetch_array($objectResult);
			return $row;
		}
		
		function getDataObject($objectResult) {
			$object = mysql_fetch_object($objectResult);
			return $object;
		}
		
		function getDataField($objectResult) {
			$field = mysql_fetch_field($objectResult);
			return $field;
		}
