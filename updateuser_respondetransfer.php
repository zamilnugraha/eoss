...
<? if($rCekTipeKodeBarang->request_by==$_GET['uid']){ ?>
	<? if($rCekTipeKodeBarang->transfer_by=='') { ?>
		...
		<br><? include("reqstore_admin_movement_transfer5.php");?>
		<? }elseif($rCekTipeKodeBarang->ticket_needassist=='NO') { ?>
		<br><? include("reqstore_admin_movement_transfer5b.php");?>
		<? } ?>
	<? }elseif($rCekTipeKodeBarang->pic_createddate!='') { ?>
...
<br>
					...
					<!--? include("reqstore_admin.php");?-->
					<? 
						$qCekTipeKodeBarangx = mysql_query("SELECT DISTINCT * FROM ITH_TIPEBARANG_KODE WHERE ticket_id = '".$_GET['pid']."' AND ticket_needassist = 'YES'");
						$rCekTipeKodeBarangx = mysql_fetch_object($qCekTipeKodeBarangx);
					?>	
						<!-- DO TRANSFER FROM ONDUTY MGR TO TECHNICIAN -->
						
							<? if($rCekTipeKodeBarangx->transfer_date==''){ ?>
								<? if($rCekTipeKodeBarangx->ticket_needassist=='YES'){ //NEED FSD TECHNICIAN ASSIST ?> 
										<br><!--NEED FSD TECHNICIAN ASSIST--><? #include("reqstore_admin_movement_transfer3.php");?>
									
										<br>TEST-Y<!--NEED FSD TECHNICIAN ASSIST--><? include("reqstore_admin_movement_transfer5.php");?>								
								<? }elseif($rCekTipeKodeBarangx->ticket_needassist=='NO'){ //NEED OPR TECHNICIAN ASSIST ?> 
										<br>TEST-Z<!--NEED FSD TECHNICIAN ASSIST--><? #include("reqstore_admin_movement_transfer3.php");?>
									
										<br><!--NEED FSD TECHNICIAN ASSIST--><? include("reqstore_admin_movement_transfer5b.php");?>								
								<? } ?>
								
							<? }elseif($rCekTipeKodeBarangx->transfer_date!=''){ ?>
