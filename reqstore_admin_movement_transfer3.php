	...
	{ ?>
		...
		<? 
			$qCekStatusKeteranganData = mysql_query("SELECT ITH_USER.user_realname, ITH_TIPEBARANG_KODE.keterangan,ITH_TIPEBARANG_KODE.ticket_id, 
										...
										ITH_USER.userstoregroup_id,ITH_TIPEBARANG_KODE.transfer_by,ITH_TIPEBARANG_KODE.pic_name,
										ITH_TIPEBARANG_KODE.received_by
	...
	<? } ?>	
		<? if($rvCekDataHeaderEQx->REQUEST_BY == $_GET['uid']){ ?>
			...
			<style>
				...
				<!--<input id="equipmentsrc" type="text" placeholder="Search..">-->
					<form name="myForm" id="myForm" target="_myFrame" action="index.php?m=form&a=amcot&pid=<?=$_GET['pid']?>&uid=<?=$_GET['uid']?>" method="POST">
		...
		{ ?>
			<? if($rvCekDataHeaderEQx->PIC_NAME != ''){ ?>
				...
				<font color="#ffffff"><center><b>SORRY, YOU HAVE SELECT THE TECHNICIANS</b></center></font>
			<? }elseif($rvCekDataHeaderEQx->PIC_NAME == ''){ ?>			
			...
			<style>
				...
				<!--<input id="equipmentsrc" type="text" placeholder="Search..">-->
				<div style="width:100%; height:auto; overflow:auto;position:relative;color:#000000;"> 
					<form name="myForm" id="myForm" target="_myFrame" action="index.php?m=form&a=amcot&pid=<?=$_GET['pid']?>&uid=<?=$_GET['uid']?>" method="POST" height="auto">
					<table class="blueTable" border="1"> 
						<thead>
							<tr>
								<?  
																...
																<? if(($rCekDataInputan->ticket_needassist=='NO')&&($rCekDataInputan->transfer_date=='')){ ?>
																		...
																		?>
																			...
																			<p name="dttransfer_by[<?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<?='dttransfer_by'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" style="font-size:10pt;margin-left:85px;margin-top:-17px;">
																					&nbsp;&nbsp; &nbsp;
																					<!--<input type="text" name="dttransfer_by[<!?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<!?='dttransfer_by'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" style="margin-left:15px;width:60%;" value=""/>-->  							
																					<select name="dttransfer_by[<?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<?='dttransfer_by'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" style="margin-left:13px;width:80%;height:auto;">
																					<option value="" style="pointer-event:none;cursor:not-allowed;">PLEASE CHOOSE</option>
																					<? 
																						$qSelectTeknisiName = mysql_query("SELECT user_nik,user_realname,userdepartemen_id,udept_id,nama_jabatan,usergroup_id,usersubgroup_id,userrsc_code,userlevel_id 
																														   FROM ITH_USER WHERE userdepartemen_id = 'OPERATIONAL_STORE' AND udept_id LIKE '%TECH%' 
