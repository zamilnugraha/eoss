...
<tbody>
	...
	<?php 
		...
		$mydeptkd = $rcekdept->kode_departemen;
			$jml_data = mysql_num_rows(mysql_query("SELECT DISTINCT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name 
			...
			LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
			 ...
			 LEFT JOIN ITH_USERGROUP ON ITH_TICKET_HEADER.ticket_createby = ITH_USERGROUP.usersubgroup_kd								 
			WHERE 
			ITH_USERGROUP.usersubgroup_kd='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
			ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
		}elseif($_SESSION['user_level'] == 4){
			$jml_data = mysql_num_rows(mysql_query("SELECT DISTINCT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
			LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
			 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
			 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id		
			 LEFT JOIN ITH_USERGROUP ON ITH_TICKET_HEADER.ticket_createby = ITH_USERGROUP.usergroup_kd
			 WHERE
			 ITH_USERGROUP.usergroup_kd='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
			ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
		}
		...
		/*if(isset($_POST['cari'])) {
			...
			*/
			...
			$dtdefault = $dayt.'/'.$montht.'/'.$yeart;
				...
				if(($_SESSION['user_level'] ==3)){			
					$query = mysql_query("SELECT DISTINCT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id		
					 LEFT JOIN ITH_USERGROUP ON ITH_TICKET_HEADER.ticket_createby = ITH_USERGROUP_AREA.usersubgroup_kd			 
					...
					echo "SELECT DISTINCT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id		
					 LEFT JOIN ITH_USERGROUP ON ITH_TICKET_HEADER.ticket_createby = ITH_USERGROUP_AREA.usersubgroup_kd			 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id		
					 LEFT JOIN ITH_USERGROUP ON ITH_TICKET_HEADER.ticket_createby = ITH_USERGROUP_REGION.usergroup_kd			 
					WHERE STR_TO_DATE(ITH_TICKET_HEADER.ticket_createdate,'%d/%m/%Y') BETWEEN STR_TO_DATE('$dfdefault','%d/%m/%Y') AND STR_TO_DATE('$dtdefault','%d/%m/%Y') 
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
					#echo 'Level = '.$_SESSION['user_level'].'Hello Admin<br>NIK ='.$_SESSION['user_nik'];	
	...
	<tr>
		...
		<?='<font color="orange">'.ucfirst(strtolower($dtmyticket->ticketstatus_name)).'</font>';?>
