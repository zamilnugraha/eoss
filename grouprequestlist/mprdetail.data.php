...
<thead>
	<tr>
		...
		<th style="background-color:#909090;color:#fff;border-color:white;width:5px;text-align:center;">Datex</th>
	...
	<?php 
		...
		or isset($_POST['statusticket_sc']) or isset($_POST['supportby_sc']) AND isset($_POST['halaman'])) 
		{
						...
						#	$romfromto_query = " AND (ITH_USERGROUP.usergroup_id BETWEEN '$_POST[romfrom_sc]' AND '$_POST[romto_sc]')";							
							$romfromto_query = " AND (ITH_USER.usergroup_id = '$_POST[romfrom_sc]')";							
						}
						...
						#	$storefromto_query = " AND (ITH_USER.userstore_id BETWEEN '$_POST[storefrom_sc]' AND '$_POST[storeto_sc]')";
							$storefromto_query = " AND (ITH_USER.user_nik = '$_POST[storefrom_sc]')";
						}
						...
						$mydeptkd = $rcekdept->kode_departemen;
							...
							ITH_USER.usergroup_id, ITH_USER.usersubgroup_id, ITH_USER.user_realname			
							...
							WHERE 
							ITH_USER.usergroup_id='$_SESSION[ugroupid]' AND ITH_USER.user_realname!='Administrator'
							$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
							$supportby_query
							ORDER BY ITH_TICKET_HEADER.ticket_createdate DESC"));
						#echo "<br>Jumlah Data = ".$jml_data.'<br>';
						/*
						echo "<br>
						SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
							ITH_USER.usergroup_id, ITH_USER.usersubgroup_id, ITH_USER.user_realname			
							FROM ITH_TICKET_HEADER 
							LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
							 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
						...
						*/
						...
						($_SESSION['user_level'] == 1001)){
							...
							LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
							 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
							 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
							WHERE ITH_TICKET_HEADER.ticket_udeptid = 'IT'			 
							ORDER BY ITH_TICKET_HEADER.ticket_createdate DESC"));
				...
				if(($_SESSION['user_level'] == 1)||($_SESSION['user_level'] == 2)||($_SESSION['user_level'] == 3)||($_SESSION['user_level'] == 4)||($_SESSION['user_level'] == 8)){				
						$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
								...
								LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
								 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
								 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
								WHERE ITH_USER.usergroup_id='$_SESSION[ugroupid]' AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
								ORDER BY ITH_TICKET_HEADER.ticket_createdate DESC LIMIT 0, $jml_per_halaman");
						...
						**/
						...
						$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.usergroup_id, ITH_USER.usersubgroup_id, ITH_USER.user_realname 
								FROM ITH_TICKET_HEADER 
								LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id 
								LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code 
								LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik 
					...
					#echo "Jumlah Per Halaman = ".$jml_per_halaman;
		...
		#} elseif((isset($_POST['cari']))OR(isset($_POST['halaman']))) {
			...
			LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
			 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
			 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik									 
			WHERE ITH_TICKET_HEADER.ticket_udeptid = 'IT' AND ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%' OR ITH_TICKET_HEADER.ticket_subject LIKE '$kunci%' 
			OR ITH_USER.user_realname LIKE '%$kunci%' AND 
			ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
			ORDER BY ITH_TICKET_HEADER.ticket_createdate DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
			#echo "TEST";
		}else{
			if(($_SESSION['user_level'] != 1)){
			...
			$dtdefault = $dayt.'/'.$montht.'/'.$yeart;	
				...
				WHERE ITH_TICKET_HEADER.ticket_udeptid = 'IT' AND STR_TO_DATE(ITH_TICKET_HEADER.ticket_createdate,'%d/%m/%Y') BETWEEN STR_TO_DATE('$dfdefault','%d/%m/%Y') AND STR_TO_DATE('$dtdefault','%d/%m/%Y') 
				ORDER BY ITH_TICKET_HEADER.ticket_createdate DESC LIMIT 0, $jml_per_halaman");
...
/**
}elseif((!isset($_POST['cari']) or !isset($_POST['romfrom_sc']) or !isset($_POST['romto_sc']) or !isset($_POST['areafrom_sc']) or !isset($_POST['areato_sc']) 
		or !isset($_POST['storefrom_sc']) or !isset($_POST['storeto_sc']) or !isset($_POST['dtfrom_sc']) or !isset($_POST['dtto_sc']) 
		or !isset($_POST['statusticket_sc']) or !isset($_POST['supportby_sc']) AND !isset($_POST['halaman']))){
**/			
?>
	...
	<? }elseif(isset($_POST['halaman'])){ ?>
		<!--?='ini halaman ='.$_POST['halaman'].'';?-->
			...
			<thead>
				<tr>
					<!--<th style="width:5px">No</th>-->
					<th style="background-color:#909090;color:#fff;border-color:white;width:5px;text-align:center;">Dates</th>
					<th style="background-color:#909090;color:#fff;border-color:white;width:5px;text-align:center;">Time</th>
					<th style="background-color:#909090;color:#fff;border-color:white;width:20px;text-align:center;">Ticket ID</th>		
					<th style="background-color:#909090;color:#fff;border-color:white;width:80px;text-align:center;">Req.By</th>
					<!--<th style="width:20px">Group</th>
							...
							if(($_SESSION['user_level'] == 1)||($_SESSION['user_level'] == 2)||($_SESSION['user_level'] == 3)||($_SESSION['user_level'] == 4)||($_SESSION['user_level'] == 8)){				
									$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
											...
											LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
											 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
											 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
											WHERE ITH_USER.usergroup_id='$_SESSION[ugroupid]'  
											AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
											$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
											$supportby_query
											ORDER BY ITH_TICKET_HEADER.ticket_createdate DESC LIMIT $i, $jml_per_halaman");
								...
								$i = ($halaman - 1) * $jml_per_halaman  + 1;
								   ...
								   $queryx = "SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
											ITH_USER.usergroup_id, ITH_USER.usersubgroup_id, ITH_USER.user_realname			
											FROM ITH_TICKET_HEADER 
											LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
											 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
...
<!--?='<br>Jumlah Halaman = '.$jml_halaman.'Jumlah Data = '.$jml_data;?-->
