...
<thead>
	<tr>
		...
		<!--? 
			$qcekstatusticketid = mysql_query("SELECT ticketstatus_id FROM ITH_TICKET_HEADER");
			$rcekstatusticketid = mysql_fetch_object($qcekstatusticketid);
			if($rcekstatusticketid->ticketstatus_id == '4'){
		?-->		
...
<tbody>
	...
	<?php 
		...
		$mydeptkd = $rcekdept->kode_departemen;
			...
			AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ticket_udeptid IN('1','2','3','4')
			...
			if($_POST['statusticket_sc']=='6'){
				if($_POST['supportby_sc']=='6'){
					...
					WHERE 
					ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
					AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ticket_udeptid ='1'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
				}elseif($_POST['supportby_sc']=='2'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name 
					FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					...
					WHERE 
					ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
					AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ticket_udeptid ='2'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name 
					FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					...
					WHERE 
					ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
					AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ticket_udeptid ='3'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name 
					FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					...
					WHERE 
					ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
					AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ticket_udeptid ='4'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
				}elseif($_POST['supportby_sc']=='7'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name 
					FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
					...
					WHERE 
					ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator'
					AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ticket_udeptid IN('2','3')
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
				}	
		...
		($_SESSION['user_level'] == 1001)){
			if($_POST['statusticket_sc']=='6'){
				if($_POST['supportby_sc']=='6'){
					...
					WHERE ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='1'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='2' AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='3' AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
				...
				if($_POST['supportby_sc']=='6'){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') AND ITH_TICKET_HEADER.ticketstatus_id ='1'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='1'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='2' AND ITH_TICKET_HEADER.ticketstatus_id ='1'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='3' AND ITH_TICKET_HEADER.ticketstatus_id ='1'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
				...
				if($_POST['supportby_sc']=='6'){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') AND ITH_TICKET_HEADER.ticketstatus_id ='2'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='1'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='2' AND ITH_TICKET_HEADER.ticketstatus_id ='2'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='3' AND ITH_TICKET_HEADER.ticketstatus_id ='2'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
				...
				if($_POST['supportby_sc']=='6'){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') AND ITH_TICKET_HEADER.ticketstatus_id ='3'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='1'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='2' AND ITH_TICKET_HEADER.ticketstatus_id ='3'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='3' AND ITH_TICKET_HEADER.ticketstatus_id ='3'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
				...
				if($_POST['supportby_sc']=='6'){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') AND ITH_TICKET_HEADER.ticketstatus_id ='0'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='1'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='2' AND ITH_TICKET_HEADER.ticketstatus_id ='0'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='3' AND ITH_TICKET_HEADER.ticketstatus_id ='0'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
				...
				if($_POST['supportby_sc']=='6'){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') AND ITH_TICKET_HEADER.ticketstatus_id ='5'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='1'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='2' AND ITH_TICKET_HEADER.ticketstatus_id ='5'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='3'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_nik
					 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
					WHERE ITH_TICKET_HEADER.ticket_udeptid ='3' AND ITH_TICKET_HEADER.ticketstatus_id ='5'
					AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%')
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
				}elseif($_POST['supportby_sc']=='4'){
					$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
					...
					if($_POST['statusticket_sc']=='6'){	
						...
						if($_POST['supportby_sc']=='6'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									AND ITH_TICKET_HEADER.ticketstatus_id = '1'	AND ticket_udeptid IN('1','2','3','4') AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='1'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '1'	AND ticket_udeptid = '1' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='2'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '1'	AND ticket_udeptid = '2' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='3'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '1'	AND ticket_udeptid = '3' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='4'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '1'	AND ticket_udeptid = '4' AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='7'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '1'	AND ticket_udeptid IN('2','3') AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}
						...
						if($_POST['supportby_sc']=='6'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '2'	AND ticket_udeptid IN('1','2','3','4') AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='1'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '2'	AND ticket_udeptid = '1' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='2'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '2'	AND ticket_udeptid = '2' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='3'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '2'	AND ticket_udeptid = '3' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='4'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '2'	AND ticket_udeptid = '4' AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='7'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '2'	AND ticket_udeptid IN('2','3') AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}					
						...
						if($_POST['supportby_sc']=='6'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '3'	AND ticket_udeptid IN('1','2','3','4') AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='1'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '3'	AND ticket_udeptid = '1' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='2'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '3'	AND ticket_udeptid = '2' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='3'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '3'	AND ticket_udeptid = '3' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='4'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '3'	AND ticket_udeptid = '4' AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='7'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '3'	AND ticket_udeptid IN('2','3') AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}
						...
						if($_POST['supportby_sc']=='6'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '0'	AND ticket_udeptid IN('1','2','3','4') AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='1'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '0'	AND ticket_udeptid = '1' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='2'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '0'	AND ticket_udeptid = '2' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='3'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '0'	AND ticket_udeptid = '3' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='4'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '0'	AND ticket_udeptid = '4' AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='7'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '0'	AND ticket_udeptid IN('2','3') AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}	
						...
						if($_POST['supportby_sc']=='6'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '5'	AND ticket_udeptid IN('1','2','3','4') AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
						}elseif($_POST['supportby_sc']=='1'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '5'	AND ticket_udeptid = '1' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='2'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '5'	AND ticket_udeptid = '2' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='3'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '5'	AND ticket_udeptid = '3' AND 
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='4'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '5'	AND ticket_udeptid = '4' AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}elseif($_POST['supportby_sc']=='7'){
							$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name	 
									...
									LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id	
									WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
									AND ITH_TICKET_HEADER.ticketstatus_id = '5'	AND ticket_udeptid IN('2','3') AND
									(ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
									$romfromto_query $areafromto_query $storefromto_query $dtfromto_query
									ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
							}
	...
	<tr style="display:show;">
		...
		<?}?></center>	
		 ...
		 <td><center>
			...
			<?='<font color="#000">IT'.ucfirst(strtolower($dtmyticket->nama_departemen)).'</font>';?>
			<?}elseif($dtmyticket->ticket_udeptid=='2'){ //Support By FSD EAST ?>
			<?='<font color="#000">FSD EAST'.ucfirst(strtolower($dtmyticket->nama_departemen)).'</font>';?>
			<?}elseif($dtmyticket->ticket_udeptid=='3'){ //Support By FSD WEST?>
			<?='<font color="#000">FSD WEST'.ucfirst(strtolower($dtmyticket->nama_departemen)).'</font>';?>
		...
		<?			
			if(($dtmyticket->ticketreferencestatus_id== NULL)&&($dtmyticket->ticketstatus_id=='0')){
		?>
		<td>
		...
		<? }elseif(($dtmyticket->ticketreferencestatus_id=='1')&&($dtmyticket->ticketstatus_id=='0')){
		?>
		<td>
			<a href="<?='http://'.$_SERVER['SERVER_NAME'].'/index.php?view=updateuserrespond&pid='.$dtmyticket->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">
		...
		<!--?='ini halaman ='.$_POST['halaman'].'<br>';?--->
			...
			<thead>
				<tr>
					...
					<th style="background-color:#909090;color:#fff;border-color:white;width:20px">Detail</th>
					<!--? 
						$qcekstatusticketidx = mysql_query("SELECT ticketstatus_id FROM ITH_TICKET_HEADER");
						$rcekstatusticketidx = mysql_fetch_object($qcekstatusticketidx);
						if($rcekstatusticketidx->ticketstatus_id == '4'){
					?-->		
						<th style="background-color:#909090;color:#fff;border-color:white;width:20px">Action</th>
					<!--? } ?-->	
				</tr>
			...
			<tbody>
					...
					($_SESSION['user_level'] == 4)||($_SESSION['user_level'] == 8)){
					$qcekdept = mysql_query("SELECT kode_departemen, nama_departemen FROM ITH_DEPARTEMEN WHERE nama_departemen='$udept'");
					$rcekdept = mysql_fetch_object($qcekdept);
					$mydeptkd = $rcekdept->kode_departemen;
						$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
						...
						if($_SESSION['statusticket']=='6')
						{
							if($_POST['supportby_sc']=='6'){					
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ITH_TICKET_HEADER.ticket_udeptid = '1'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='2'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ITH_TICKET_HEADER.ticket_udeptid = '2'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='3'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ITH_TICKET_HEADER.ticket_udeptid = '3'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='4'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ITH_TICKET_HEADER.ticket_udeptid = '4'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='7'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5') AND ITH_TICKET_HEADER.ticket_udeptid IN('2','3')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}
						...
						}elseif($_SESSION['statusticket']=='1')
						{
							if($_POST['supportby_sc']=='6'){					
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '1' AND ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));				
							}elseif($_POST['supportby_sc']=='1'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '1' AND ITH_TICKET_HEADER.ticket_udeptid = '1'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='2'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '1' AND ITH_TICKET_HEADER.ticket_udeptid = '2'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='3'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '1' AND ITH_TICKET_HEADER.ticket_udeptid = '3'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='4'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '1' AND ITH_TICKET_HEADER.ticket_udeptid = '4'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='7'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '1' AND ITH_TICKET_HEADER.ticket_udeptid IN('2','3')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}
						...
						}elseif($_SESSION['statusticket']=='2')
						{
							if($_POST['supportby_sc']=='6'){					
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '2' AND ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));					
							}elseif($_POST['supportby_sc']=='1'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '2' AND ITH_TICKET_HEADER.ticket_udeptid = '1'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='2'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '2' AND ITH_TICKET_HEADER.ticket_udeptid = '2'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='3'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '2' AND ITH_TICKET_HEADER.ticket_udeptid = '3'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='4'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '2' AND ITH_TICKET_HEADER.ticket_udeptid = '4'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='7'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '2' AND ITH_TICKET_HEADER.ticket_udeptid IN('2','3')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}
						}elseif($_SESSION['statusticket']=='3')
						{
							if($_POST['supportby_sc']=='6'){					
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '3' AND ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));		
							}elseif($_POST['supportby_sc']=='1'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '3' AND ITH_TICKET_HEADER.ticket_udeptid = '1'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='2'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '3' AND ITH_TICKET_HEADER.ticket_udeptid = '2'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='3'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '3' AND ITH_TICKET_HEADER.ticket_udeptid = '3'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='4'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '3' AND ITH_TICKET_HEADER.ticket_udeptid = '4'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='7'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '3' AND ITH_TICKET_HEADER.ticket_udeptid IN('2','3')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}					
						...
						}elseif($_SESSION['statusticket']=='0')
						{
							if($_POST['supportby_sc']=='6'){					
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '0' AND ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));				
							}elseif($_POST['supportby_sc']=='1'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '0' AND ITH_TICKET_HEADER.ticket_udeptid = '1'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='2'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '0' AND ITH_TICKET_HEADER.ticket_udeptid = '2'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='3'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '0' AND ITH_TICKET_HEADER.ticket_udeptid = '3'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='4'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '0' AND ITH_TICKET_HEADER.ticket_udeptid = '4'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='7'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '0' AND ITH_TICKET_HEADER.ticket_udeptid IN('2','3')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}
						}elseif($_SESSION['statusticket']=='5')
						{
							if($_POST['supportby_sc']=='6'){					
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '5' AND ITH_TICKET_HEADER.ticket_udeptid = '1'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='2'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '5' AND ITH_TICKET_HEADER.ticket_udeptid = '2'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='3'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '5' AND ITH_TICKET_HEADER.ticket_udeptid = '3'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));	
							}elseif($_POST['supportby_sc']=='4'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '5' AND ITH_TICKET_HEADER.ticket_udeptid = '4'
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}elseif($_POST['supportby_sc']=='7'){					
								$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_TICKETREFERENCESTATUS.ticketreferencestatus_name 
								...
								WHERE 
								ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_USER.user_realname!='Administrator' 
								AND ITH_TICKET_HEADER.ticketstatus_id = '5' AND ITH_TICKET_HEADER.ticket_udeptid IN('2','3')
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query 							 
								ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
							}
						}							
								...
								$i = ($halaman - 1) * $jml_per_halaman  + 0;
										...
										$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
												...
												LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
												 ...
												 LEFT JOIN ITH_DEPARTEMEN ON ITH_TICKET_HEADER.ticket_udeptid = ITH_DEPARTEMEN.kode_departemen	
												 LEFT JOIN ITH_TICKETREFERENCESTATUS ON ITH_TICKET_HEADER.ticketreferencestatus_id = ITH_TICKETREFERENCESTATUS.ticketreferencestatus_id
												WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' AND ITH_TICKET_HEADER.ticket_udeptid IN('1','2','3','4') 
							    ...
							    if($_SESSION['statusticket']=='6'){
									if($_POST['supportby_sc']=='6'){
										...
										$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
												...
												WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
												AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
												AND ITH_TICKET_HEADER.ticket_udeptid = '1'
												AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
												$romfromto_query $areafromto_query $storefromto_query $dtfromto_query											
												ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
									}elseif($_POST['supportby_sc']=='2'){
										$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
												...
												WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
												AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
												AND ITH_TICKET_HEADER.ticket_udeptid = '2'
												AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
												$romfromto_query $areafromto_query $storefromto_query $dtfromto_query											
												ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
									}elseif($_POST['supportby_sc']=='3'){
										$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
												...
												WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
												AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
												AND ITH_TICKET_HEADER.ticket_udeptid = '3'
												AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
												$romfromto_query $areafromto_query $storefromto_query $dtfromto_query											
												ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
									}elseif($_POST['supportby_sc']=='4'){
										$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
												...
												WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
												AND ITH_TICKET_HEADER.ticketstatus_id IN ('1','2','3','0','5')
												AND ITH_TICKET_HEADER.ticket_udeptid = '4'
												AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
												$romfromto_query $areafromto_query $storefromto_query $dtfromto_query											
												ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
									}elseif($_POST['supportby_sc']=='7'){
									...
									if($_POST['supportby_sc']=='6'){
										$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, 
												...
												WHERE ITH_TICKET_HEADER.ticket_createby='$_SESSION[user_nik]' 
												AND ITH_TICKET_HEADER.ticketstatus_id = '1'
												AND ITH_TICKET_HEADER.ticket_udeptid IN ('1','2','3','4')
												AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
												$romfromto_query $areafromto_query $storefromto_query $dtfromto_query											
												ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
									}elseif($_POST['supportby_sc']=='1'){
				...
				<tr>
					...
					<?}?></center>	
					 ...
					 <td>	<center>		
						...
						<?='<font color="black">'.ucfirst(strtoupper($dtmyticketx->nama_departemen)).'</font>';?>
					...
					<? }elseif($_SESSION['user_level']== 1){ ?>
						<?			
							if(($dtmyticketx->ticketreferencestatus_id== NULL)&&($dtmyticketx->ticketstatus_id=='0')){
						?>
						<td>
						...
						<? }elseif(($dtmyticketx->ticketreferencestatus_id=='1')&&($dtmyticketx->ticketstatus_id=='0')){
						?>
						<td>
							<a href="<?='http://'.$_SERVER['SERVER_NAME'].'/index.php?view=updateuserrespond&pid='.$dtmyticketx->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticketx->ticket_id?>" target="_parent">
								<img src="img/reference.png" alt="" height="30" width="50"></img>
...
<div class="pagination pagination-right">
  ...
  <ul>
  		...
  		<li class="halaman" id="1"><a href="#">firsts</a></li>
