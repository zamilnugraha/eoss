...
<? if($rCekTipeKodeBarang->request_by==$_GET['uid']){ ?>
		<br><? /* include("reqstore_admin_movement_transfer.php"); */ include("reqstore_admin_movement.php"); ?>
...
?>
<? if($rCekTipeKodeBarang->request_by!=$_GET['uid']){ ?>
	<? if($rCekTipeKodeBarang->transfer_by!='') { ?>
		<!--? if($rCekTipeKodeBarang->ticket_needassist=='YES') { ?-->
		<!--<br><!? include("reqstore_admin_movement_transfer6.php");?>
		<!? }elseif($rCekTipeKodeBarang->ticket_needassist=='NO') { ?>
		<br><!? include("reqstore_admin_movement_transfer6b.php");?>
		<!? } ?-->
			<? if(($rCekTipeKodeBarang->ticket_needassist=='YES')||($rCekTipeKodeBarang->ticket_needassist=='NO')) { ?>
			<?
				$qCekItemRcv = mysql_query("SELECT * FROM ITH_TIPEBARANG_KODE WHERE ticket_id = '".$_GET['pid']."'");
				while($rCekItemRcv = mysql_fetch_object($qCekItemRcv)){
					#echo "<hr>Kode Barang = ".$rCekItemRcv->kode_tipebarang;
					#echo "<br>Received Date = ".$rCekItemRcv->received_date;
					#echo "<br>SELECT ticket_id,received_date,kode_tipebarang FROM ITH_TIPEBARANG_KODE 
				    #       <br>                 WHERE ticket_id = '".$_GET['pid']."' 
					#			<br>			    AND kode_tipebarang = '".$rCekItemRcv->kode_tipebarang."'
					#		<br>  AND received_date = '".$rCekItemRcv->received_date."'";
				$qCekDataRcv = mysql_query("SELECT ticket_id,received_date,received_by,kode_tipebarang,tickettransferto_outletname FROM ITH_TIPEBARANG_KODE 
				                            WHERE ticket_id = '".$_GET['pid']."' 
										    AND kode_tipebarang = '".$rCekItemRcv->kode_tipebarang."' 
											AND received_date = '".$rCekItemRcv->received_date."'");
				}
				#while($rCekDataRcv = mysql_fetch_object($qCekDataRcv)){
				$rCekDataRcv = mysql_fetch_object($qCekDataRcv);	
			?>
				<!--?='<br>Cek Data = '.$rCekDataRcv->received_date;?-->
				<? if($rCekDataRcv->received_date==''){?>	
					<? if($rCekDataRcv->tickettransferto_outletname=='-'){?>
						<br>WWW1<? include("reqstore_admin_movement_transfer6c.php");?>		
					<? }elseif($rCekDataRcv->tickettransferto_outletname!='-'){ ?>
						<? if(($rCekDataRcv->tickettransferto_outletcode==$_SESSION['userstoregroup_id'])&&($rCekDataRcv->received_date=='')||($rCekDataRcv->received_date=='-')){?>
						<br>WWW1X<? include("reqstore_admin_movement_transfer6e.php");?>	
						<? }elseif(($rCekDataRcv->tickettransferto_outletcode==$_SESSION['userstoregroup_id'])&&($rCekDataRcv->received_date!='')||($rCekDataRcv->received_date!='-')){ ?>
						<br>WWW1Z<? include("reqstore_admin_movement_transfer6e.php");?>	
						<? } ?>	
					<? } ?>	
				<? }elseif($rCekDataRcv->received_date=='-'){?>
					<br>WWW2<? include("reqstore_admin_movement_transfer6d.php");?>		
				<? }elseif(($rCekDataRcv->received_date=='-')||($rCekDataRcv->received_date=='')){ ?> 
					<br>WWW3<? include("reqstore_admin_movement_transfer6d.php");?>
				<? }else{ ?> <br>WWW4<? include("reqstore_admin_movement_transfer6d.php");?>
				<? } ?>
			<? #} ?>
			<? }elseif($rCekTipeKodeBarang->ticket_needassist=='YES') { ?>
			<br>GGG2<? include("reqstore_admin_movement_transfer6.php");?>
			<? }elseif($rCekTipeKodeBarang->ticket_needassist=='NO') { ?>
			<br>GGG3<? include("reqstore_admin_movement_transfer6b.php");?>
			<? } ?>		
	<? }elseif($rCekTipeKodeBarang->transfer_by=='') { ?>
		<br>
		<center><image src="images/approveds2.png" style="text-align:center;background:(#7fd9fc, #6576aa);"></image><br>
		<font color="#ffffff"><b><u>sorry, the handover from the store hasn't been done yet</b></center></font>
...
<div style="display:none;">
		<?
			...
			if($rcekimg->id_img ==''){
		?>
			<font color="#ffffff"><b><u>X1 Image Uploaded By Store/User</u><br><center>Sorry, No Image Uploaded.</b></center></font>
		<? }else{ ?>
		...
		<?
			...
			if($rcekimg->id_img ==''){
		?>
			<font color="#ffffff"><b><u>X2 Image Uploaded By PIC</u><br><center>Sorry, No Image Uploaded.</b></center></font>
		<? }else{ ?>
...
<br>
					...
					<!--? include("reqstore_admin.php");?-->
					<? 
						$qCekTipeKodeBarangx = mysql_query("SELECT DISTINCT * FROM ITH_TIPEBARANG_KODE 
															WHERE ticket_id = '".$_GET['pid']."' AND ticket_needassist IN('YES','NO')
															AND kode_tipebrg IN('RQS-03-000112move','RQS-03-000113move','RQS-03-000114move')");
						$rCekTipeKodeBarangx = mysql_fetch_object($qCekTipeKodeBarangx);
						#while($rCekTipeKodeBarangx = mysql_fetch_object($qCekTipeKodeBarangx)){
					?>	
						<!-- DO TRANSFER FROM ONDUTY MGR TO TECHNICIAN -->
				
							<? if((($rCekTipeKodeBarangx->received_date=='')||($rCekTipeKodeBarangx->received_date=='-'))
								&&(($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000112move')||($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000113move')
							||($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000114move'))){ ?>
								<? if(($rCekTipeKodeBarangx->ticket_needassist=='YES')||($rCekTipeKodeBarangx->ticket_needassist=='NO')){ //NEED FSD TECHNICIAN ASSIST ?> 
									<br>Z1<!--NEED FSD TECHNICIAN ASSIST--><!--? include("reqstore_admin_movement_transfer5c.php");?-->
									<? include("reqstore_admin_movement_transfer6d.php");?>		
								<? }elseif($rCekTipeKodeBarangx->ticket_needassist=='YES'){ //NEED FSD TECHNICIAN ASSIST ?> 
										<br>Z2<!--NEED FSD TECHNICIAN ASSIST--><? #include("reqstore_admin_movement_transfer3.php");?>
									
										<br><!--NEED FSD TECHNICIAN ASSIST--><? include("reqstore_admin_movement_transfer5.php");?>								
								<? }elseif($rCekTipeKodeBarangx->ticket_needassist=='NO'){ //NEED FSD TECHNICIAN ASSIST ?> 
										<br><!--NEED FSD TECHNICIAN ASSIST--><? #include("reqstore_admin_movement_transfer3.php");?>
									
										<br>Z3<!--NEED FSD TECHNICIAN ASSIST--><? include("reqstore_admin_movement_transfer5.php");?>								
								<? } ?>
							<? }elseif(($rCekTipeKodeBarangx->pictakein_by!='')&&(($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000112move')||($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000113move')
							||($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000114move'))){ ?>
								<? if(($rCekTipeKodeBarangx->ticket_needassist=='YES')||($rCekTipeKodeBarangx->ticket_needassist=='NO')){ //NEED FSD TECHNICIAN ASSIST ?> 
									<br>Z1<!--NEED FSD TECHNICIAN ASSIST--><? include("reqstore_admin_movement_transfer5c.php");?>	
								<? }elseif($rCekTipeKodeBarangx->ticket_needassist=='YES'){ //NEED FSD TECHNICIAN ASSIST ?> 
							...
							<? }elseif((($rCekTipeKodeBarangx->received_date!='')||($rCekTipeKodeBarangx->received_date!='-'))
							&&(($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000112move')||($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000113move')
							||($rCekTipeKodeBarangx->kode_tipebrg=='RQS-03-000114move'))){ ?>
								<?if($rCekTipeKodeBarangx->pictakein_by!='-'){?>
									<br><!--NEED FSD TECHNICIAN ASSIST--><? #include("reqstore_admin_movement_transfer5c.php");?>								<?}elseif($rCekTipeKodeBarangx->pictakein_by=='-'){?>
								<center><image src="images/approveds2.png" style="text-align:center;background:(#7fd9fc, #6576aa);"></image><br>
	...
	<br>					
		<?
			...
			if($rcekimg->id_img ==''){
		?>
			<font color="#ffffff"><b><u>X3 Image Uploaded By Store/User</u><br><center>Sorry, No Image Uploaded.</b></center></font>
		...
		<?
			...
			if($rcekimg->id_img ==''){
		?>
			<font color="#ffffff"><b><u>X4 Image Uploaded By PIC</u><br><center>Sorry, No Image Uploaded.</b></center></font>
		<? }else{ ?>
			<b><font color="Blue">Image Uploaded By PIC</font></b>
			<link rel="stylesheet" href="css/w3.css">
			<div style="overflow-y:auto; width:100%;"> 
