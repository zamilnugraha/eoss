...
<tbody>
	...
	<?php 
		...
		}elseif(($_SESSION['user_level'] == 99)||($_SESSION['user_level'] == 100)){
			if(($_SESSION['user_dept'] == 1)){
				...
				LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
				 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
				 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
				 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id	WHERE ITH_TICKET_HEADER.ticket_type = '1'				 
				ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));
			}elseif(($_SESSION['user_dept'] == 2)){
				$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
				LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
				 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
				 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
				 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id	WHERE ITH_TICKET_HEADER.ticket_type = '2'			
				ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));			
			}elseif(($_SESSION['user_dept'] == 3)){
				$jml_data = mysql_num_rows(mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
				LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
				 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
				 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
				 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id	WHERE ITH_TICKET_HEADER.ticket_type = '3'	
				ORDER BY ITH_TICKET_HEADER.ticket_no DESC"));			
			}	
		}
					...
					*/		
					...
					if(($_SESSION['user_dept'] == 1)){				
						$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name FROM ITH_TICKET_HEADER 
								...
								LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
								LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
								ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id	
								LEFT JOIN ITH_USERGROUP ON ITH_USER.userlevel_id = ITH_USERGROUP.userlevel_id
								WHERE ITH_TICKET_HEADER.ticket_type='1' AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
								ORDER BY ITH_TICKET_HEADER.ticket_no ASC");
					}elseif(($_SESSION['user_dept'] == 2)){
						$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name FROM ITH_TICKET_HEADER 
								...
								ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id	
								LEFT JOIN ITH_USERGROUP ON ITH_USER.userlevel_id = ITH_USERGROUP.userlevel_id
								WHERE ITH_TICKET_HEADER.ticket_type='2' AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
								ORDER BY ITH_TICKET_HEADER.ticket_no ASC");
					}elseif(($_SESSION['user_dept'] == 3)){	
						$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.usersubgroup2_name FROM ITH_TICKET_HEADER 
								...
								ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id	
								LEFT JOIN ITH_USERGROUP ON ITH_USER.userlevel_id = ITH_USERGROUP.userlevel_id
								WHERE ITH_TICKET_HEADER.ticket_type='3' AND (ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%') 
								$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
								ORDER BY ITH_TICKET_HEADER.ticket_no ASC");					
					}
						/*
		...
		/*if(isset($_POST['cari'])) {
			...
			if(($_SESSION['user_level'] == 1)){			
				...
				LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
				 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
				 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id									 
				WHERE ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%' OR ITH_TICKET_HEADER.ticket_subject LIKE '$kunci%' 
				...
				if(($_SESSION['user_dept'] == 1)){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
					 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id									 
					WHERE ITH_TICKET_HEADER.ticket_type='1' AND ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%' OR ITH_TICKET_HEADER.ticket_subject LIKE '$kunci%' 
					OR ITH_USER.user_realname LIKE '%$kunci%' 
					$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
				}elseif(($_SESSION['user_dept'] == 2)){
					$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
					 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id									 
					WHERE ITH_TICKET_HEADER.ticket_type='2' AND ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%' OR ITH_TICKET_HEADER.ticket_subject LIKE '$kunci%' 
					OR ITH_USER.user_realname LIKE '%$kunci%' 
					$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
				}elseif(($_SESSION['user_dept'] == 3)){
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
					 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id									 
					WHERE ITH_TICKET_HEADER.ticket_type='3' AND ITH_TICKET_HEADER.ticket_id LIKE '%$kunci%' OR ITH_TICKET_HEADER.ticket_subject LIKE '$kunci%' 
					OR ITH_USER.user_realname LIKE '%$kunci%' 
					$romfromto_query $areafromto_query $storefromto_query $dtfromto_query $tktsts_query
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
				}	
			...
			$dtdefault = $dayt.'/'.$montht.'/'.$yeart;	
				if(($_SESSION['user_dept'] == 1)){			
					...
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
					 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id								 
					 WHERE ITH_TICKET_HEADER.ticket_type='1' AND STR_TO_DATE(ITH_TICKET_HEADER.ticket_createdate,'%d/%m/%Y') BETWEEN STR_TO_DATE('$dfdefault','%d/%m/%Y') AND STR_TO_DATE('$dtdefault','%d/%m/%Y') 
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
				}elseif(($_SESSION['user_dept'] == 2)){
					$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
					 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id								 
					WHERE ITH_TICKET_HEADER.ticket_type='2' AND STR_TO_DATE(ITH_TICKET_HEADER.ticket_createdate,'%d/%m/%Y') BETWEEN STR_TO_DATE('$dfdefault','%d/%m/%Y') AND STR_TO_DATE('$dtdefault','%d/%m/%Y') 
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");				
				}elseif(($_SESSION['user_dept'] == 3)){
					$query = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
					LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
					 ...
					 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id AND
					 ITH_TICKET_HEADER.ticket_type = ITH_USER.departemen_id								 
					WHERE ITH_TICKET_HEADER.ticket_type='3' AND STR_TO_DATE(ITH_TICKET_HEADER.ticket_createdate,'%d/%m/%Y') BETWEEN STR_TO_DATE('$dfdefault','%d/%m/%Y') AND STR_TO_DATE('$dtdefault','%d/%m/%Y') 
					ORDER BY ITH_TICKET_HEADER.ticket_no DESC LIMIT 0, $jml_per_halaman");
				}
