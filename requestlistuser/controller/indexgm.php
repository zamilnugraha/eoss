...
$query = "SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name, ITH_DEPARTEMEN.nama_departemen 
				...
				WHERE 1=1 AND 
				ITH_TICKET_HEADER.ticket_notestatusapp3 = 'Request Ini Perlu Persetujuan Dari GM OPERATION Terlebih Dahulu, 
				Karena Harga Per Item Melebihi Batas Nilai Yang Telah Ditentukan. Terimakasih' 
				".$queryAnd."
...
LEFT JOIN ITH_DEPARTEMEN ON ITH_TICKET_HEADER.ticket_udeptid = ITH_DEPARTEMEN.kode_departemen
WHERE 1=1 AND 
ITH_TICKET_HEADER.ticket_notestatusapp3 = 'Request Ini Perlu Persetujuan Dari GM OPERATION Terlebih Dahulu, Karena Harga Per Item Melebihi Batas Nilai Yang Telah Ditentukan. Terimakasih'
AND STR_TO_DATE(ITH_TICKET_HEADER.ticket_createdate, '%d/%m/%Y') 
BETWEEN STR_TO_DATE('13/12/2018', '%d/%m/%Y') AND STR_TO_DATE('13/12/2018', '%d/%m/%Y')
GROUP BY ITH_TICKET_HEADER.ticket_id
ORDER BY ITH_TICKET_HEADER.ticket_createdate, ITH_TICKET_HEADER.ticket_createtime DESC";
//echo $query;				
