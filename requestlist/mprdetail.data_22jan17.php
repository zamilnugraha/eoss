...
$udept = $_SESSION['user_departemen'];
 		$pid = $_GET['pid']; $uid = $_GET['username'];
?>
<!--<script>
...
<thead>
	<tr>
		...
		<th style="background-color:#909090;color:#fff;border-color:white;width:20px;text-align:center;">User Feedback</th>
...
<tbody>
	...
	<tr>
		...
		<?}?>
		 ...
		 <? } ?>
		<td>
		<?if($dtmyticket->ticketuserfeedback_id=='1'){ //Open Status ?> 
		<?='<font color="red">'.ucfirst(strtolower($dtmyticket->user_realname)).'</font>';?>
		<?}elseif($dtmyticket->ticketuserfeedback_id=='0'){ //Solved Status ?>
		<?='<font color="blue">'.ucfirst(strtolower($dtmyticket->user_realname)).'</font>';?>
		<?}elseif($dtmyticket->ticketuserfeedback_id=='2'){ //On Process Status?>
		<?='<font color="Green">'.ucfirst(strtolower($dtmyticket->user_realname)).'</font>';?>
		<?}elseif($dtmyticket->ticketuserfeedback_id=='3'){ //Cancel Status?>
		<?='<font color="#000">'.ucfirst(strtolower($dtmyticket->user_realname)).'</font>';?>
		<?}?>
		</td>		 
		<td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=comment&pid='.$dtmyticket->ticket_id.'&username='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">Readmore</a></td>		
