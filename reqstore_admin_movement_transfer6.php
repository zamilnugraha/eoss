	...
	{ ?>
		...
		<? 
			$qCekStatusKeteranganData = mysql_query("SELECT ITH_USER.user_realname, ITH_TIPEBARANG_KODE.keterangan,ITH_TIPEBARANG_KODE.ticket_needassist,ITH_TIPEBARANG_KODE.ticket_id, 
										...
										WHERE ITH_TIPEBARANG_KODE.ticket_id = '".$_GET['pid']."' AND transfer_date NOT IN('')");
		...
		<? }elseif(($detailmyticketKeteranganData->keterangan=='MOVE TO WAREHOUSE')){ ?>
			...
			<? if($detailmyticketKeteranganData->ticket_needassist=='YES'){ //ASSIGN TECHNICIAN FROM FSD MANAGER ?>
				<? if($_SESSION['departemen_id']==5){ ?>
					<font color="#ffffff"><b><u>WILL RECEIVE GOODS FROM STORES TO TECHNICIANS<!--SERAH TERIMA BARANG DARI TOKO KE TEKNISI --> </u></b></font>
				<? }elseif($_SESSION['departemen_id']==3){ ?>
					<font color="#ffffff"><b><u>WILL RECEIVE GOODS FROM TECHNICIANS TO WAREHOUSE<!--SERAH TERIMA BARANG DARI TEKNISI KE WAREHOUSE STAFF --> </u></b></font>
				<? } ?>
												...
												{	
															...
															**/
															$qvCekDataEQx = mysql_query("SELECT DISTINCT ITH_TIPEBARANG_KODE.TICKET_ID, ITH_TIPEBARANG_KODE.NO_PERMINTAAN,
																						...
																						ITH_TIPEBARANG_KODE.ticket_needassist IN('YES') AND transfer_date NOT IN('') AND
																...
																*/	$qvSCatReplace2 = "SELECT DISTINCT 
																...
																<? 
																	$qCekDataInputan = mysql_query("SELECT received_by, received_date, tickettransferto_outletcode, tickettransferto_outletname 
																									FROM ITH_TIPEBARANG_KODE 
																									WHERE ticket_id ='".$_GET['pid']."' 
																									 AND transfer_date NOT IN('')
																									 AND tickettransferto_outletcode = '".$_GET['uid']."'");
														...
														{    #echo "<br>SOLVED";			
															...
															<tr>												
																...
																*/	$qvSCatReplace2 = "SELECT DISTINCT 
																...
																<? if(($rCekDataInputan->ticket_needassist=='YES')&&($rCekDataInputan->received_date=='')){ ?>
																		<? 
																			$mydate = date('d/m/Y', strtotime('today - 30 days'));
																			#$mydate = date('d/m/Y');
																			/*$mydate = date('d/m/Y', strtotime('today + 30 days'));*/
																...
																<? }elseif(($rCekDataInputan->ticket_needassist=='YES')&&($rCekDataInputan->received_date!='')){ ?>
