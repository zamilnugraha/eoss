...
?>
	...
	{ ?>
		...
		<? 
			$qCekStatusKeteranganData = mysql_query("SELECT ITH_USER.user_realname, ITH_TIPEBARANG_KODE.keterangan, ITH_TIPEBARANG_KODE.tickettransferto_outletcode 
										...
										AND ITH_TIPEBARANG_KODE.tickettransferto_outletcode = '".$_SESSION['user_nik']."'");
		...
		<? }elseif(($detailmyticketKeteranganData->keterangan=='MOVE TO OTHER STORE')&&($detailmyticketKeteranganData->tickettransferto_outletcode==$_SESSION['user_nik'])){ ?>
			<font color="#ffffff"><b><u>RECEIVE MOVEMENT ITEM FROM STORE <?=$detailmyticketKeteranganData->user_realname?> </u></b></font>
		<? } ?>			
		...
		?>	
			...
			<style>
				...
				<!--<input id="equipmentsrc" type="text" placeholder="Search..">-->
					<form name="myForm" id="myForm" target="_myFrame" action="index.php?m=form&a=srvfb&pid=<?=$_GET['pid']?>&uid=<?=$_GET['uid']?>" method="POST">
												...
												{	
															...
															**/
															$qvCekDataEQx = mysql_query("SELECT DISTINCT ITH_TIPEBARANG_KODE.TICKET_ID, ITH_TIPEBARANG_KODE.NO_PERMINTAAN,
																						...
																						ITH_TIPEBARANG_KODE.TRANSFER_BY, ITH_TIPEBARANG_KODE.TRANSFER_DATE
																						FROM ITH_TIPEBARANG_KODE  
														...
														{    #echo "<br>SOLVED";			
															...
															<tr>												
																...
																*/	$qvSCatReplace2 = "SELECT DISTINCT 
																	...
																	$pemisah_ribuan  = ".";	
																?>		
																<div style="margin-top:-35px;display:none;"><b><u>REPLACEMENT ITEM : </u></b></div>
																<!--<ul style="margin-bottom:-5px;">-->
																<ul style="margin-top:-35px;">
																<li><b> Tagging Number : <font color="brown"><?=$rvSCatReplace2->NOTAGGING;?></font></b></li>
																<li style="display:none;"><b> <font color="brown"><?=$rvSCatReplace2->ITEMCODEREPLACE;?> - <?=$rvSCatReplace2->ITEMNAMEREPLACE;?></font></b></li>
																...
																<? 
																	$qCekDataInputan = mysql_query("SELECT received_by, received_date, tickettransferto_outletcode, tickettransferto_outletname,
																									transfer_date, transfer_by, ticket_needassist
																									FROM ITH_TIPEBARANG_KODE WHERE ticket_id ='".$_GET['pid']."' 
																									AND tickettransferto_outletcode = '".$_GET['uid']."'");
																	...
																	$transferredbyName = preg_replace('/[^A-Za-z0-9\  ]/', '', $rvCekDataEQx['TRANSFER_BY']);
																	$transferredbyNames = ucwords(strtolower($transferredbyName));									
																?>
																<? if($rCekDataInputan->received_date==''){ ?>
																	...
																	?>
																	<? 
																	$qCekDataInputanView = mysql_query("SELECT received_by, received_date, tickettransferto_outletcode, tickettransferto_outletname,
																									transfer_date, transfer_by, ticket_needassist
																									FROM ITH_TIPEBARANG_KODE WHERE ticket_id ='".$_GET['pid']."' 
																									AND tickettransferto_outletcode = '".$_GET['uid']."'");
																	$rCekDataInputanView = mysql_fetch_object($qCekDataInputanView);
																	?>
																	<? if($rvCekDataEQx['TICKET_NEEDASSIST'] == 'YES'){?>
																	&nbsp;&nbsp;<b>Item&nbsp;Status&nbsp;:</b>&nbsp;&nbsp;<font color="brown"><b><u><?='Transferred By FSD Technician';?></u></b></font><br>
																	<? }elseif($rvCekDataEQx['TICKET_NEEDASSIST'] == 'NO'){?>
																	&nbsp;&nbsp;<b>Item&nbsp;Status&nbsp;:</b>&nbsp;&nbsp;<font color="brown"><b><u><?='Transferred By OPR Technician';?></u></b></font><br>
																	<? } ?>
																	&nbsp;&nbsp;<b>Transferred&nbsp;Date&nbsp;:</b>&nbsp;&nbsp;<font color="brown"><?=$rCekDataInputanView->transfer_date?></font><br>
																	&nbsp;&nbsp;<b>Transferred&nbsp;By&nbsp;:</b>&nbsp;&nbsp;<font color="brown"><?=strtoupper($transferredbyNames)?></font><br><br><hr>		
																	
																		<input type="hidden" name="kdtipebrg[]" id="<?='kdtipebrg'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" value="<?=$rvCekDataEQx["KODE_TIPEBARANG"]?>"></input>
																		&nbsp;&nbsp;<b>Received&nbsp;Date:</b>
																		<p name="dtreceived_sc[<?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<?='dtreceived_sc'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" style="font-size:10pt;margin-left:85px;margin-top:-17px;">
																				&nbsp;&nbsp; &nbsp;<input type="text" name="dtreceived_sc[<?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<?='dtreceived_sc'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" class="date start" style="margin-left:15px;width:60%;" value="<?=$datenow?>"/>  							
																		</p>
																		&nbsp;&nbsp;<b>Received&nbsp;By:</b>
																		<p name="dtreceived_by[<?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<?='dtreceived_by'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" style="font-size:10pt;margin-left:85px;margin-top:-17px;">
																				&nbsp;&nbsp; &nbsp;<input type="text" name="dtreceived_by[<?=$rvCekDataEQx["KODE_TIPEBARANG"];?>]" id="<?='dtreceived_by'.$rvCekDataEQx["KODE_TIPEBARANG"];?>" style="margin-left:15px;width:60%;" value=""/>  							
																		</p>
																		<script>
																			...
																			$('#<?='dtreceived_sc'.$rvCekDataEQx["KODE_TIPEBARANG"];?> .date').datepicker({
																				/*'format': 'd MM yyyy',*/
																				'format': 'dd/mm/yyyy',
																				'autoclose': true
																			});

																			$('#<?='dtreceived_sc'.$rvCekDataEQx["KODE_TIPEBARANG"];?>').datepair();
																		</script>
																<? }elseif($rCekDataInputan->received_date!=''){ ?>	
																	&nbsp;&nbsp;<b>Item&nbsp;Status&nbsp;:</b>&nbsp;&nbsp;<?='Received By Store '.ucwords(strtolower($rCekDataInputan->tickettransferto_outletname))?><br>
																	&nbsp;&nbsp;<b>Received&nbsp;Date&nbsp;:</b>&nbsp;&nbsp;<?=$rCekDataInputan->received_date?><br>
																	&nbsp;&nbsp;<b>Received&nbsp;By&nbsp;:</b>&nbsp;&nbsp;<?=strtoupper($receivedbyNames)?>		
																<? } ?>			
