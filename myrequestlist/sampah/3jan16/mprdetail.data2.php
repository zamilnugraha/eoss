...
<tbody>
	...
	<?php 
		...
		isset($_POST['halaman'])) 
		{
						...
						//filter ROM From
						if($_POST['romfrom_sc'] !=NULL ){
							$romfrom = $_POST['romfrom_sc'];
							$romfrom_query = " AND b.usergroup_name LIKE '%$romfrom%' ";
							if($romfrom == 'Rom I-1'){
								$romf = "dan rom(From) Name = ROM I-1";
							}elseif($romfrom == 'Rom I-2'){
						...
						//filter ROM To
						if($_POST['romto_sc'] != NULL ){
							$romto = $_POST['romto_sc'];
							$romto_query = " AND b.usergroup_name LIKE '%$romto%' ";
							if($romto == 'Rom I-1'){
								$romt = "dan rom(To) Name = ROM I-1";
							}elseif($romto == 'Rom I-2'){
						...
						//filter Area From
						if($_POST['areafrom_sc'] != NULL ){
							$areafrom = $_POST['areafrom_sc'];
							$areafrom_query = " AND b.usersubgroup_name LIKE '%$areafrom%' ";
						...
						//filter ROM To
						if($_POST['areato_sc'] != NULL ){
							$areato = $_POST['areato_sc'];
							$areato_query = " AND b.usersubgroup_name LIKE '%$areato%' ";
							if($areato == 'Area I-1'){
						...
						//filter Store From
						if($_POST['storefrom_sc'] != NULL ){
							$storefrom = $_POST['storefrom_sc'];
							$storefrom_query = " AND b.userstore_name LIKE '%$storefrom%' ";
						...
						//filter Store To
						if($_POST['storeto_sc'] !=NULL ){
							$storeto = $_POST['storeto_sc'];
							$storeto_query = " AND b.userstore_name LIKE '%$storeto%' ";
						...
						//filter Date From
						if($_POST['datefrom_sc'] !='' ){
							$datefrom = $_POST['datefrom_sc'];
							$dtfrom_query = " AND (a.ticket_createdate >= '$datefrom'";
						...
						//filter Date To
						if($_POST['dateto_sc'] !='' ){
							$dateto = $_POST['dateto_sc'];
							$dtto_query2 = " AND a.ticket_createdate <= '$dateto')";
						...
						if($_POST['statusticket_sc'] !='' ){
							...
							$tktsts_query = " AND a.ticketstatus_id <= '$statusticket')";
						...
						echo "<strong>Hasil pencarian untuk kata kunci $kunci $romfrom $romto $areafrom $areato $storefrom $storeto $datefrom $dateto $statusticket </strong>";
					/**	echo "<strong>Hasil pencarian untuk kata kunci $kunci $cat $pyr $tv </strong>
						...
						$query = mysql_query("SELECT ITH_TICKET.*, ITH_TICKETSTATUS.ticketstatus_name, ITH_USER.user_realname, ITH_USERGROUP.usergroup_name, ITH_USERGROUP.usersubgroup_name, ITH_USERGROUP.userstore_name FROM ITH_TICKET 
								...
								WHERE (ITH_TICKET.ticket_subject LIKE '$kunci%') $romfrom_query $romto_query $areafrom_query $areato_query $storefrom_query $storeto_query $datefrom_query $dateto_query $tktsts_query
								AND ITH_TICKET.ticket_createby='$_SESSION[user_realname]'
