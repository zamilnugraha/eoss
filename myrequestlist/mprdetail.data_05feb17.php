...
<thead>
	<tr>
		...
		<th style="background-color:#909090;color:#fff;border-color:white;width:80px;text-align:center;">Request By</th>
		...
		<th style="background-color:#909090;color:#fff;border-color:white;width:20px;text-align:center;">Request Via</th>
		<th style="background-color:#909090;color:#fff;border-color:white;width:20px">View Detail</th>
...
<tbody>
	...
	<tr style="display:show;">
		...
		<? if($_SESSION['user_level']!= 1){?>	
			<? 
				if(($dtmyticket->ticket_viaby=='By Email')||($dtmyticket->ticket_viaby=='By Phone')){
			?>					
				<!-- <td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=update&pid='.$dtmyticket->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">Update</a></td>-->
				...
				<img src="img\editx.png" alt="" height="30" width="50"></img>
				</a></td>		
			<? }elseif(($dtmyticket->ticket_viaby=='Via Web')||($dtmyticket->ticket_viaby=='Via Web')){ ?>
				<!-- <td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=updateforuser&pid='.$dtmyticket->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">Update</a></td>-->
				<td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=updateforuser&pid='.$dtmyticket->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">
				<img src="img\editx.png" alt="" height="30" width="50"></img>
				</a></td>	
			<? } ?>		<? } ?>
		<?php  $i++;	
			...
			<tbody>
					...
					or isset($_POST['statusticket_sc']) AND isset($_POST['halaman'])) 
					{
						...
						**/			
						...
						$queryx = mysql_query("SELECT ITH_TICKET_HEADER.*, ITH_USER.user_realname, ITH_TICKETSTATUS.ticketstatus_name FROM ITH_TICKET_HEADER 
						LEFT JOIN ITH_TICKETSTATUS ON ITH_TICKET_HEADER.ticketstatus_id = ITH_TICKETSTATUS.ticketstatus_id
						 LEFT JOIN ITH_TICKETCATEGORYNEW ON ITH_TICKET_HEADER.ticketsubcategory_code = ITH_TICKETCATEGORYNEW.ticketsubcategory2_code
						 LEFT JOIN ITH_USER ON ITH_TICKET_HEADER.ticket_createby = ITH_USER.user_id									 
						WHERE ITH_TICKET_HEADER.ticket_udeptid = 'IT' AND ITH_TICKET_HEADER.ticketstatus_id = '1'
				...
				<tr style="display:show;">
					...
					<?='<font color="red">'.ucfirst(strtolower($dtmyticketx->ticketstatus_name)).'</font>';?>
					...
					<?}?>
					 ...
					 <? }elseif(($dtmyticketx->ticket_viaby=='By Web')||($dtmyticketx->ticket_viaby=='')){?>
						<td><?='By Web';?></td>
					 <? } ?>	
					<td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=comment&pid='.$dtmyticketx->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">Readmore</a></td>	
					<? if($_SESSION['user_level']!= 1){?>
						<? 
							if($dtmyticketx->ticket_viaby!='Via Web'){
						?>					
							<td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=update&pid='.$dtmyticketx->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">Update</a></td>		
						<? }elseif($dtmyticketx->ticket_viaby=='Via Web'){ ?>
							<td><a href="<?='http://'.$_SERVER['SERVER_NAME'].':85/helpdesk1/index.php?view=updateforuser&pid='.$dtmyticketx->ticket_id.'&uid='.$_SESSION['user_nik']?>" title="Ticket ID: <?=$dtmyticket->ticket_id?>" target="_parent">Update</a></td>								
						<? } ?>
					<? } ?>
						<?php  $i++;	
					}
