...
?>
			...
			<style>
				...
				?>				
					<b><u>ACTIVITY TRACKING [REFERENCE TICKET : (<?='<font color="blue"><b>'.$rCekStatusTicketReffx->ticketreference_id.'</b></font>';?>)]</u></b>
							...
							?>
									...
									if(($rcekoldticket->ticketstatus_id=='2')||($rcekoldticket->ticketstatus_id!=2))
									{
										...
										$qcekdtl = mysql_query("SELECT ticket_id, ticketstatus_name, ticketstatusapproval_name, ticketreport_subject, ticketreport_createddate, ticketreport_createdtime, ticketrespond_by FROM VW_DETAIL_TRACKING WHERE ticket_id ='$_GET[pid]' 
													...
													ASC;");
										***/
										$qcekdtl = mysql_query("SELECT ticket_id, ticketstatus_name, ticketstatusapproval_name, ticketreport_subject, ticketreport_createddate, 
													ticketreport_createdtime, ticketrespond_by FROM VW_DETAIL_TRACKING WHERE ticket_id ='$_GET[pid]' 
									...
									?>								
										<tr>
											...
											<td width="7%" style="display:none;"><center><?=$i++;?></center></td>
											...
											{ ?>	
												...
												<td width="15%"><center>3<?='Admin Assignment';?></center></td>
											<? 
											...
											<? }elseif(($rcekstatustracking->userlevel_id == '1')||($rcekstatustracking->userlevel_id == '2')||
												...
												?>	
													<td width="25%"><center>Open</center></td>
												<? }elseif($rCekStatusTicketID1->ticketstatus_id=='2'){ ?>	
													<td width="25%"><center>On Process</center></td>
												<? }elseif($rCekStatusTicketID1->ticketstatus_id=='0'){ ?>	
													<td width="25%"><center>Solved</center></td>
												<? }elseif($rCekStatusTicketID1->ticketstatus_id=='5'){ ?>	
													<td width="25%"><center>Closed</center></td>
												<? }elseif($rCekStatusTicketID1->ticketstatus_id=='3'){ ?>	
													<td width="25%"><center>Cancel</center></td>
												<? } ?>	
												<td width="15%"><center><?=$rcekdtl->ticketrespond_by;?></center></td>
									...
									?>								
										<tr>
											<td style="display:none;"><center><?=$rcekdtl->logreport_id;?></center></td>
											<td width="17%"><center>222<?=$rcekdtl->ticket_id;?></center></td>
											<td width="10%"><center><?=$rcekdtl->ticketreport_createddate;?></center></td>
											<td width="10%"><center><?=$rcekdtl->ticketreport_createdtime;?></center></td>
											<? 
												$qcekstatustracking = mysql_query("SELECT user_nik, user_realname, userlevel_id, nama_jabatan, udept_id 
								...
								{ ?>		
									...
									****/
										...
										$qcekdtl = mysql_query("SELECT ticket_id, ticketstatus_name, ticketstatusapproval_name, ticketreport_subject, ticketreport_createddate, 
													ticketreport_createdtime, ticketrespond_by FROM VW_DETAIL_TRACKING 
													WHERE ticket_id IN('".$rCekTicketReff->ticketreference_id."','".$_GET['pid']."') 
													GROUP BY  ticket_id, ticketstatus_name, ticketstatusapproval_name, ticketreport_subject, ticketreport_createddate, 
													ticketreport_createdtime, ticketrespond_by
													ORDER BY ticket_id, STR_TO_DATE(ticketreport_createddate,'%d/%m/%Y'), DATE_FORMAT(CAST(ticketreport_createdtime AS TIME), '%H %i') 
													ASC");
										
										/*$qcekdtl = mysql_query("SELECT ticket_id, ticketstatus_name, ticketstatusapproval_name, ticketreport_subject, ticketreport_createddate, 
													ticketreport_createdtime, ticketrespond_by FROM VW_DETAIL_TRACKING 
													WHERE ticket_id IN('".$rCekTicketReff->ticketreference_id."') 
													GROUP BY  ticket_id, ticketstatus_name, ticketstatusapproval_name, ticketreport_subject,  STR_TO_DATE(ticketreport_createddate,'%d/%m/%Y'), 
													DATE_FORMAT(CAST(ticketreport_createdtime AS TIME), '%H %i'), ticketrespond_by
													ORDER BY STR_TO_DATE(ticketreport_createddate,'%d/%m/%Y'), DATE_FORMAT(CAST(ticketreport_createdtime AS TIME), '%H %i')	ASC");	
										*/
										while($rcekdtl = mysql_fetch_object($qcekdtl))
										{
											$ticketrespondby = $rcekdtl->ticketrespond_by;
